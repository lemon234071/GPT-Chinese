...
def eval_bleu(ref_resp, hyps_resp):
    ...
    for i in range(4):
        weights = [1/(i+1)]*(i+1)
        bleu.append(corpus_bleu(ref_resp, hyps_resp, weights=weights,
...
if __name__ == '__main__':
    ...
    golden = load_txt(golden_path)

    # [[word,...,word],...,[word,...,word]]
    infer = [line.strip().split() for line in infer]
    # [[[word,...,word]],...,[[word,...,word]]]
    golden = [[line.strip().split()] for line in golden]

    random_bleu = [round(x, 5) for x in eval_bleu(golden, infer)]
